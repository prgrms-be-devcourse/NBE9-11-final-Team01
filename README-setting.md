...
> 이 문서는 코드 품질 도구 · 모니터링 · 부하 테스트 실행 설정을 다룹니다.
> 프로젝트 소개는 [README.md](./README.md)를 참고하세요.
...
최소 커버리지 기준은 **라인 70%(임시)** 이며, 미달 패키지 보완 후 최종 목표인 **80%**로 상향할 예정입니다. 아래 패키지는 측정에서 제외됩니다.
...
| `com.develop.snaptix.staff.*` | deprecated 모듈 |
...
로컬에서 `docker-compose up -d`(MySQL + Redis)를 실행한 뒤 앱을 기동합니다.
...
메트릭 접두어는 `snaptix_*`로 통일되어 있으며, 카운터는 부트 시 0-초기화되어 "미배포"와 "처리 0건"을 구분할 수 있습니다. 큐 백로그 깊이는 `snaptix_order_stream_xlen` / `snaptix_order_stream_xpending` 게이지로 관측합니다.
...
> 운영(AWS) 환경에서는 `/actuator/prometheus`를 기존 도메인에서 차단하고, 별도 A레코드 도메인 + NPM Access List(Basic Auth) + HTTPS + 경로 제한으로만 노출합니다.
