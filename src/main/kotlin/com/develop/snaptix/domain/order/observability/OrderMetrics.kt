...
object OrderMetrics {
     ...
     */
    const val QUEUE_SIZE = "snaptix.order.queue.size"

    /**
     * 백프레셔 429 반환 수 (Counter).
     * XLEN ≥ 정원+α 시 +1. BackpressureGuard 에서 등록.
     */
    const val BACKPRESSURE_COUNT = "snaptix.order.backpressure.count"

    /**
     * PEL 적체 수 (Gauge).
     * XAUTOCLAIM 주기마다 갱신. 증가 추세 → 유실/워커 지연 조기 신호.
     */
    const val PENDING_SIZE = "snaptix.stream.pending.size"

    /**
     * XAUTOCLAIM 회수·재처리 수 (Counter).
     * OrphanReclaimer 에서 등록.
     */
    const val CLAIM_REPROCESS_COUNT = "snaptix.order.claim.reprocess.count"

    /**
     * 트림 유실(XAUTOCLAIM deletedIds 비어있지 않음) 수 (Counter).
     * 0 초과 즉시 CRITICAL. OrphanReclaimer 에서 등록.
     */
    const val DELETED_COUNT = "snaptix.stream.deleted.count"

    /**
     * 재고 보상 +1 회수 수 (Counter).
     * CompensationService 보상 성공마다 +1.
     */
    const val COMPENSATE_COUNT = "snaptix.stock.compensate.count"

    /**
     * 정상 처리 완료(XACK) 수 (Counter).
     * OrderStreamConsumer ACK 성공마다 +1.
     */
    const val XACK_COUNT = "snaptix.order.xack.count"
}
