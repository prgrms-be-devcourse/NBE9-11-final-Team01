...
class EventController(
    ...
    fun getEvents(
        ...
        @Parameter(description = "정렬 기준 (startTime, createdAt, name)", example = "startTime") @RequestParam(defaultValue = "startTime") sortBy: String,
        @Parameter(description = "정렬 방향 (asc, desc)", example = "asc") @RequestParam(defaultValue = "asc") sortDir: String,
        @Parameter(description = "장소 필터링", required = false) @RequestParam(required = false) location: String?,
        @Parameter(description = "시작일 필터링 (yyyy-MM-dd)", required = false) @RequestParam(required = false) startDate: LocalDate?,
        @Parameter(description = "종료일 필터링 (yyyy-MM-dd)", required = false) @RequestParam(required = false) endDate: LocalDate?
    ...
    @GetMapping("/{eventId}")
    fun getEventDetail(
        @Parameter(description = "이벤트 공개 ID (UUID)", required = true) @PathVariable eventId: String
