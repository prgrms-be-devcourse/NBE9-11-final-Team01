 ...
 * - SSE TICKET_ISSUED 발행은 [com.develop.snaptix.domain.ticket.service.TicketService] 로
 *   이관됐으므로 이 테스트에서 검증하지 않는다.
 *
 * ## 커버하는 AC
 * 1. 정상 흐름: 4단계 모두 올바른 인자로 호출
 * 2. eventPublicId 조회 실패 → markCompleted 생략, 나머지 단계는 실행
 * 3. 각 단계 실패 → 예외 미전파, 이후 단계 계속 실행 (soft-fail 연쇄 검증)
 * 4. 단계 순서: findById → removeClaimed → markCompleted → delete
 */
...
class PaymentConfirmRedisCleanupServiceTest {
    ...
    // removeClaimed/delete는 Unit 반환 — relaxed mock으로 명시 스텁 불필요
    ...
    inner class StepOrdering {
        @Test
        @DisplayName("eventId 조회 → removeClaimed → markCompleted → DEL 순서로 실행된다")
        ...
        @Test
        @DisplayName("markCompleted(3단계)는 ORDER_HOLD DEL(4단계)보다 먼저 수행된다")
        fun `markCompleted precedes orderHold delete`() {
            sut.cleanup(orderId, zoneId, userId, internalEventId)
